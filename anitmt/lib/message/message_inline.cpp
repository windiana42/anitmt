...
namespace message
{
  ...
  inline int Message_Consultant::get_verbose_level() 
  { 
    return verbose_level; 
  }
  inline bool Message_Consultant::is_verbose(int level) 
  { 
    return verbose_level >= level; 
  }
  inline bool Message_Consultant::is_warning() 
  { 
    return warnings; 
  }

  inline void Message_Consultant::set_msg_source
  ( Message_Source_Identifier src )
  {
    msg_source = src;
  }
  inline void Message_Consultant::set_verbose_level( int level )
  {
    verbose_level = level;
  }
  inline void Message_Consultant::set_warnings( bool warn )
  {
    warnings = warn;
  }

  inline int Message_Consultant::verbose_indent(int delta)
  {
    vindent+=delta;
    if(vindent<0)  vindent=0;
    return(vindent);
  }
  inline void Message_Consultant::verbose_indent_set(int val)
  {
    vindent=val;
    if(vindent<0)  vindent=0;
  }
