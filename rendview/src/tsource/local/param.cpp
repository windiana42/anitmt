...
int TaskSourceFactory_Local::_CheckFramePattern(
	...
	int may_miss_spec)
{
	// NOTE: We do NOT skip "%%" here. Because other code 
	//       simply does if(strchr(pattern,'%')) ... and would 
	//       thus be tricked. 
...
{
	bool dumped_renderer=0,dumped_filter=0;
	...
	//if(fi->rdesc)
	{
		...
		#else
		if(!compare_to || fi->use_clock!=compare_to->use_clock)
		{
			Verbose(TSI,"      Use frame clock: %s",
				fi->use_clock ? "yes" : "no");
		}
		#endif
		_VPrintFrameInfo_DumpListIfNeeded(
			dumped_renderer ? "      Add args" : "      Add render args",
			compare_to ? &compare_to->radd_args : NULL,&fi->radd_args);
		_VPrintFrameInfo_DumpListIfNeeded(
			dumped_renderer ? "      Add files" : "      Add render files",
	...
	//if(fi->fdesc)
	{
		...
		_VPrintFrameInfo_DumpListIfNeeded(
			dumped_filter ? "      Add args" : "      Add filter args",
			compare_to ? &compare_to->fadd_args : NULL,&fi->fadd_args);
		_VPrintFrameInfo_DumpListIfNeeded(
			dumped_filter ? "      Add files" : "      Add filter files",
...
#if 0  	/* FrameClockVal: UNUSED */
	_PFBMergeHelper(_PFBMergeDouble(&dest->clock_step,
		sa->clock_step,sa->set_flags & SF_clock_step,
		sb->clock_step,sb->set_flags & SF_clock_step),
		SF_clock_step,&failed,&dest->set_flags);
static int warned=0;
if(dest->use_clock && !warned)
{ // #####FIXME#####
Error("local/param.cpp:%d: MISSING in frame info: merge clock_start values.\n"
	"(BUG; please suggest how you would expect it to work;)\n",
	__LINE__);  ++warned;
}  // For now, simply override: 
	...
	const PerFrameTaskInfo *a,const PerFrameTaskInfo *b)
{
	...
	return(
		...
		//!_different_clock(a,b) &&   	/* FrameClockVal: UNUSED */
		a->use_clock == b->use_clock && 
...
int TaskSourceFactory_Local::_FixupPerFrameBlock(PerFrameTaskInfo *fi)
{
	...
	if(fi->rdesc && !(fi->set_flags & SF_use_clock))
	{
		...
		// Do not do that, otherwise you have to specify -r-fc for EVERY 
		// per-frame block to ever use frame clock. 
		//else
		//{  fi->use_clock=false;  }
	}
	#endif
	if(fi->rdesc && fi->use_clock)
	{
		...
		{
			(*(is_error ? &MessageLogger::_Error : &MessageLogger::_Warning))(
...
int TaskSourceFactory_Local::FinalInit()
{
	...
	if(!mfailed)
	{
		// In case the number of frames is not specified and we 
		// are using an input frame pattern without %d-spec, write 
		// an error. We might otherwise never stop rendering...
		// (I only check the master_fi here because we sooner or 
		// later have so high frame numbers that only the master is 
		// left. There is no sense in checking a per-frame block 
		// because this block may consist of only 1 frame and we 
		// do not bail out before 3 frames fail. Or it contains 3 
		// frames but the fjump is 2. Or...)
		if(master_fi.nframes<0 && !strchr(master_fi.rinfpattern.str(),'%'))
		{
			Error("Local: Number of frames unspecified while master frame "
				"block has -r-infpattern without %%d-spec.\n");
			++mfailed;
		}
	}
	...
	// Okay, now, the fi->ii stuff no longer exists and we did set 
	...
	if(!failed)	
	{
		...
		for(PerFrameTaskInfo *fi=fi_list.first(); fi; fi=fi->next)
		{
			...
			// For clock, use simple overriding. 
			...
			// Check frame patterns (and use defaults from master if needed): 
	...
	if(!failed)
	{
		...
		Verbose(TSI,"  Continuing: %s  (add files: %s mtime)\n",
			cont_flag ? "yes" : "no",
			check_add_mtime ? "check" : "ignore");
...
int TaskSourceFactory_Local::parse(const Section * /*s*/,PAR::SPHInfo *info)
{
	...
	Verbose(DBG,"Adding new per-frame block %d:%d (last frame: %d).\n",
		...
		new_fi->first_frame_no+new_fi->nframes-1);
...
int TaskSourceFactory_Local::_RegisterParams()
{
	...
	AddParam("add-check-mtime|acmt",
		"when -cont enabled: check modification time of additional files "
		"and re-render/filter if modified (default).",
		&check_add_mtime);
	...
	int show_in_help)
{
	...
	ParamInfo *tmp=AddParam("rcont",
		...
		"the operation mode; you must use -cont to actually continue. "
	...
	AddParam("r-ifpattern","render input file pattern (e.g. f%07d.pov; "
		...
		"use same file for every frame) (relative to r-dir)",
	...
	AddParam("r-ofpattern","render output file pattern (e.g. f%07d; "
		...
		"extension added according to output format (relative to r-dir)",
		fi ? &fi->routfpattern : NULL,flags);
	tmp=AddParam("r-frameclock|r-fc","use frame clock value to be passed "
		"to renderer; should also pass +KFI +KFF +KI +KF to POVRay.",
		fi ? &fi->use_clock : NULL,PNoDefault | flags);
	if(fi)  fi->ii->use_frameclock_pi=tmp;
	#if 0  	/* FrameClockVal: UNUSED */
	AddParam("r-fcstart|r-fc0","frame clock value of first frame",
		fi ? &fi->clock_start : NULL,flags);
	AddParam("r-fcstep|r-fcdt","frame clock step between two frames",
	...
	AddParam("f-ofpattern","filter output file pattern (e.g. f%07d-f.png; "
		"must use %d,%x,%X to be replaced by frame number) (relative to f-dir)",
...
const char *TaskSourceFactory_Local::SF_FlagString(int flag)
{
	switch(flag)
	{
		...
		//case SF_clock_start:   return("fcstart");  	/* FrameClockVal: UNUSED */
		//case SF_clock_step:    return("fcstep");  	/* FrameClockVal: UNUSED */
	}
	return("???");
}
	...
	fadd_files(failflag)
{
	...
	use_clock=false;
	//clock_start=NAN;  	/* FrameClockVal: UNUSED */
	//clock_step=NAN;  	/* FrameClockVal: UNUSED */
	...
	fadd_files(failflag)
{
	...
	//clock_start=c->clock_start;  	/* FrameClockVal: UNUSED */
	//clock_step=c->clock_step;  	/* FrameClockVal: UNUSED */
...
TaskSourceFactory_Local::PerFrameTaskInfo_Internal::PerFrameTaskInfo_Internal(
	int *failflag) :
