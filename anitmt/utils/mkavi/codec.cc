...
void codec::prescan(image *p, int framenum)
{
...
void codec::set_head_size( long size ){
  head = size + 4;
}

void codec::set_index( long size, char *tname )
{
  idx[actidx].pos   = actpos;
  idx[actidx].size  = size;
  idx[actidx].tname = tname;
  actidx++;
  actpos += size + 8;

  if( actidx >= nframes )
    {
      cerr << "Conflicting number of frames!!!" << endl;
      actidx--;
    }
}

void codec::write_index()
{
  char fname[44];
  ...
  for( int i=0; i<nframes; i++ )
    {
      sprintf(fname, "index of frame#%d", i);

      f = new index_item_chunk( this->cs, 0 != i, fname, idx[i].tname , 
				idx[i].pos + head, idx[i].size );
      f->write();
    }
}
...
void rgb24::frame(image *p, int framenum)
{
  assert(p->w() == this->w && p->h() == this->h);
  int sz = (p->w() + (p->w() & 1)) * p->h() * 3;
  unsigned char *bytes = new unsigned char[sz + 100000 /*redundat space*/];
  ...
  while(row >= 0) {
    ...
    // reverse red and blue component to bgr colors
    unsigned char *ppt = pt;
    for( int i=0; i< p->w(); i++ ) {
      unsigned char buf = ppt[0];
      ppt[0] = ppt[2];
      ppt[2] = buf;
      ppt+=3;
    }      
...
void cram16::frame(image *p, int framenum)
{
  assert(p->w() % 4 == 0);
  assert(p->h() % 4 == 0);
  assert(p->w() == this->w && p->h() == this->h);
...
void cram16::c8quad(unsigned char *&bp, image *p, int x, int y, int setflag,
...
void cram16::c2block(unsigned char *&bp, image *p, int x, int y,
