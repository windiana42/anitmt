...
char *Frame_Dump::Node::_Dump_Object2Str(char *d,char *dend,Frame_Dump::Context *ctx)
{
	...
	{
		++ctx->active_objects;
		scale_flag =(Frame_Dump::Dump_Flags)(scale_flag | DF_Obj_AScale);
		rot_flag   =(Frame_Dump::Dump_Flags)(rot_flag | DF_Obj_ARot);
		trans_flag =(Frame_Dump::Dump_Flags)(trans_flag | DF_Obj_ATrans);
	}
	...
	Dump_Flags flags,unsigned int id)
{
	...
	// Tweak flags: 
	if(flags & DF_Obj_Scale)  flags=(Frame_Dump::Dump_Flags)(flags | DF_Obj_AScale);
	if(flags & DF_Obj_Rot)    flags=(Frame_Dump::Dump_Flags)(flags | DF_Obj_ARot);
	if(flags & DF_Obj_Trans)  flags=(Frame_Dump::Dump_Flags)(flags | DF_Obj_ATrans);
