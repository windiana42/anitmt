...
struct StaticInfoPerType
{
	...
	const char *_sh_stopsig_desc_str;
};
...
static StaticInfoPerType static_ift[_DTLast]=
{
	// DTRender: 
	{
		...
		"stop signal to use (\"STOP\" or \"TSTP\")",
	},
	// DTFilter:
	{
		"fdfile",
...
int ComponentDataBase::ReadInDescFiles(par::ParameterSource_File *file_src)
{
	...
	for(int dtype=0; dtype<_DTLast; dtype++)
	{
		...
		//L o c k P a r a m(dti->_descfile_pi,1);
...
// Appends string!
// separate entries by sep string. 
	...
	const SpecialHelpItem *shi)
{
	...
	else if(shi->item_id==-2)
	{
		...
		RefString tmp;
		tmp.set("-opmode specifies task source and task driver.\n"
			"List of supported operation modes with corresponding "
			"source and driver names:\n");
		dest->append(tmp);
		...
		if(p->do_dump_info)
		{
			...
			for(const RF_DescBase *i=p->desclist.first(); i; i=i->next)
			{
				...
				tmp.sprintf(0,"\r6:Driver:  %s",
					...
					dest->append(tmp);
				tmp.sprintf(0,"\r6:StopSig: SIG%s",
					i->stop_sig_to_send==SIGTSTP ? "TSTP" : (
					i->stop_sig_to_send==SIGSTOP ? "STOP" : "????") );
					dest->append(tmp);
...
int ComponentDataBase::CheckParams()
{
	...
	Verbose(MiscInfo,"Final desc setup: ");
	...
	for(int _dtype=0; _dtype<_DTLast; _dtype++)
	{
		...
		{  Verbose(MiscInfo,"[%s: ",DTypeString(dtype));  }
		...
		for(RF_DescBase *i=dti->desclist.first(); i; i=i->next)
		{
			...
			if(!dname)
			{
				if(!err_failed && !warn_failed)  Verbose(MiscInfo,_vfailed_str);
			...
			if(i->_stopsigname.str())
			{
				const char *str=i->_stopsigname.str();
				if(!strcasecmp(str,"SIGTSTP") || !strcasecmp(str,"TSTP"))
				{  i->stop_sig_to_send=SIGTSTP;  }
				else if(!strcasecmp(str,"SIGSTOP") || !strcasecmp(str,"STOP"))
				{  i->stop_sig_to_send=SIGSTOP;  }
				else
				{
					if(!err_failed && !warn_failed)  Verbose(MiscInfo,_vfailed_str);
					Error("Illegal stopsig spec \"%s\" for %s desc \"%s\".\n",
						str,DTypeString(dtype),i->name.str());
					++err_failed;
					continue;
				}
				
				// Free memory: 
				i->_stopsigname.deref();
			}
		...
		if(!err_failed && !warn_failed)
		{  Verbose(MiscInfo,"%d entries] ",nent);  }
...
void ComponentDataBase::_SetAdminPort(RendViewAdminPort *ap)
{
	assert(!adminprt);
	adminprt=ap;
}
...
ComponentDataBase::ComponentDataBase(par::ParameterManager *pman,
	ProcessManager *_procman,int *failflag) : 
...
RF_DescBase::RF_DescBase(int *failflag) : 
	...
	_stopsigname(failflag)
{
	dfactory=NULL;
	stop_sig_to_send=SIGTSTP;
}
