...
namespace anitmt{
  ...
  class Return {
  private:
    Return_Type *prev, *next;

    // friend functions for the according container class
    friend Contain_Return<Return_Type>;
    void set_prev( Return_Type* prev );
    void set_next( Return_Type* next );
  protected:
    Return_Type *get_prev( Return_Type type_ID = Return_Type() );
    Return_Type *get_next( Return_Type type_ID = Return_Type() );
  public:
